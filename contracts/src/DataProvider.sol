...
contract DataProvider {
    ...
    function getUserTokens(IStabilanCore core, address account) external view returns (UserToken[] memory) {
        ...
        for (uint256 i = 0; i < optionTokens.length; i++) {
            ...
            if (balance > 0) {
                userTokensTemp[userTokenLen++] =
                    UserToken({tokenType: TokenType.OPTION, tokenAddress: address(optionTokens[i]), balance: balance});
            }
        }
        ...
        for (uint256 i = 0; i < backingTokens.length; i++) {
            ...
            if (balance > 0) {
                userTokensTemp[userTokenLen++] =
                    UserToken({tokenType: TokenType.BACKING, tokenAddress: address(backingTokens[i]), balance: balance});
            }
        }
